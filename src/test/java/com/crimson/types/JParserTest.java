...
public class JParserTest {
  ...
  public void verifyForkJoinPoolParserAccess_Test() {
    // Verify parser access via ForkJoin pool (parallel stream). This test makes more sense when the
    // number of CPUs is much
    // smaller than the size of the "inputs" array since for Fork Join pool based on parallel
    // streams the number of threads
    // is typically set equal to the number of CPU cores available.
    Arrays.asList(inputs).parallelStream().forEach(this::createJsonObject);
  }
  ...
  @Test
  public void verifyWorkerThreadPoolParserAccess_Test()
      throws InterruptedException, ExecutionException, TimeoutException {
    // Create custom thread pool with two threads
    ExecutorService service = Executors.newFixedThreadPool(2);

    // Use an array of completable futures to gather futures submitted to the above worker thread
    // pool of size 2
